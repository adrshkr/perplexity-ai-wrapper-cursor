...
class PerplexityClient:
    ...
    def __init__(
        ...
        # Initialize cloudscraper bypass if available and enabled
        if self.use_cloudflare_bypass and CloudflareBypass:
            try:
                ...
                # Extract cookies from cloudscraper and apply to httpx client
                if cookies:
                    self.bypass.update_cookies(cookies)
            except Exception as e:
                # Fallback to regular httpx if cloudscraper fails
                import warnings

                warnings.warn(
                    f"Cloudflare bypass initialization failed: {e}. Falling back to regular httpx.",
                    UserWarning,
                )
                self.use_cloudflare_bypass = False
                self.bypass = None

        # Update client with cookies if provided
        if cookies:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                with self.client.stream(
                    ...
                    json=payload,
                    timeout=httpx.Timeout(self.timeout * 2),
                ) as response:
                        ...
                        if isinstance(chunk, dict):
                            ...
                            if chunk.get("step_type") == "FINAL" and "content" in chunk:
                                    ...
                                    if isinstance(answer_str, str):
                                        try:
                                            ...
                                            if (
                                                ...
                                                accumulated_data["text"] = answer_obj[
                                                    "answer"
                                                ]
                                ...
                                if isinstance(content, dict):
                                    ...
                                    if "chunks" in content and isinstance(
                                        content["chunks"], list
                                    ):
                                        answer_parts = []
                                        ...
                                        if answer_parts:
                                            accumulated_data["text"] = "".join(
                                                answer_parts
                                            )  # Join without spaces
                                            break
                                    # Check structured_answer
                                    elif (
                                        "structured_answer" in content
                                        and content["structured_answer"]
                                    ):
                                        if (
                                            isinstance(
                                                content["structured_answer"], list
                                            )
                                            and len(content["structured_answer"]) > 0
                                        ):
    ...
    def _handle_response(
        self, response: httpx.Response, query: str, data: Optional[Dict] = None
    ) -> SearchResponse:
        """Handle API response"""
        if data is not None:
        ...
        if response.status_code == 200:
            ...
            self._handle_error_response(response)
            # This should never be reached due to exception above, but for type safety
            raise NetworkError("Response handling failed")
    ...
    def _handle_error_response(self, response: httpx.Response) -> None:
        ...
        if response.status_code == 403:
            ...
            if (
                "cloudflare" in response_text_lower
                or "just a moment" in response_text_lower
                or "challenge" in response_text_lower
            ):
    ...
    def _parse_response(self, data: Dict, query: str) -> SearchResponse:
                        ...
                        if isinstance(content, dict):
                            ...
                            if "answer" in content:
                                ...
                                if isinstance(answer_val, str):
                                    ...
                                    isinstance(answer_val, dict)
                                    and "answer" in answer_val
                                ):
        ...
        if not answer:
            ...
            if not answer and "answer" in data:
                ...
                if isinstance(answer_val, str):
                    ...
                    if not (
                        answer_val.strip().startswith("[")
                        or answer_val.strip().startswith("{")
                    ):
                        answer = answer_val
            ...
            if not answer and "response" in data:
                ...
                if isinstance(response_val, dict):
                    ...
                    answer = (
                        str(response_val)
                        if not (
                            str(response_val).strip().startswith("[")
                            or str(response_val).strip().startswith("{")
                        )
                        else ""
                    )
        ...
        if "_all_chunks" in data:
            ...
            for chunk in all_chunks:
                if (
                    isinstance(chunk, dict)
                    and chunk.get("step_type") == "SEARCH_RESULTS"
                ):
                    content = chunk.get("content", {})
                    if isinstance(content, dict):
                        # Extract web_results
        ...
        # Extract model
        model = (
            data.get("display_model")
            or data.get("model_preference")
            or data.get("model")
        )
        ...
        conversation_id = (
            data.get("uuid")
            or data.get("context_uuid")
            or data.get("frontend_context_uuid")
        )
    ...
    def create_labs_project(
        self, query: str, project_type: str = "document", **kwargs
    ) -> SearchResponse:
        """
        ...
        payload = {"query": query, "type": project_type, "mode": "labs", **kwargs}
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if is_cloudflare_challenge(response):
                    raise NetworkError(
                        "Cloudflare challenge detected for Labs endpoint. "
                        "Labs features require valid authentication cookies.\n\n"
                        "SOLUTION:\n"
                        "1. Extract cookies from logged-in browser:\n"
                        "   .\\perplexity.bat cookies extract --profile labs_user\n"
                        "2. Use browser automation instead:\n"
                        "   from automation.web_driver import PerplexityWebDriver\n"
                        "  driver = PerplexityWebDriver()\n"
                        "  driver.start()\n"
                        "  driver.navigate_to_perplexity()\n"
        ...
        # If we get here, all retries failed
        raise NetworkError("Labs project creation failed - all retries exhausted")
